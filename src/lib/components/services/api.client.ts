...
async function request<T>(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { error: true, status: response.status, data: null, message: data.message || 'Error en la petición' };
        }

        return { error: false, status: response.status, data: data as T };
    } catch (err) {
        return { error: true, status: 0, data: null, message: 'Error de red o CORS no configurado en el BFF', details: err };
    }
}
